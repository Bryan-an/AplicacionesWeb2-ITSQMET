...
const updatePermissionState = (data, updatedState) => {
  const permission = JSON.parse(data);
  const { id, start_date, end_date, type, observation } = permission;
...
const deletePermission = (id) => {
  let response = confirm("¿Estás seguro de eliminar esta tarea?");
          ...
          <td class="text-center">
            ...
            <button type="button" class="btn btn-danger" onclick="deletePermission('${id}')">
              Eliminar
                ...
                : `
            <button type="button" class="btn btn-primary mr-1" onclick='updatePermissionState(\`${JSON.stringify(
              permission
            )}\`, "aprobado")'>
              Aprobar
            </button>
            <button type="button" class="btn btn-danger" onclick='updatePermissionState(\`${JSON.stringify(
              permission
            )}\`, "rechazado")'>
              Rechazar
            </button>`
